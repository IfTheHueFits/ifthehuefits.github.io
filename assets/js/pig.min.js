(function(global) {
	...
	function Pig(imageData, options) {
		...
		this.settings = {
			...
			/**
			 * Type: array
			 * Default: [20, 100, 600, 100]
			 * Description: The sizes available for each image to be loaded.
			 */
			sizes: [20, 100, 600, 100],
			...
			urlForSize: function(filename, size) {
				return 'generated/assets/images/' + filename + '-' + size;
			},
			 ...
			 * `lastWindowWidth`.
			 *
			 * @param {Number} thumbnailHeight - The last computed width of the
			 *                                   thumbnail image.
			 *
			 * @returns {Number} The size (height in pixels) of the images to load.
			 */
			getImageSize: function(thumbnailHeight) {
				var size
				for (size in this.sizes){
					if (thumbnailHeight <= this.sizes[size])
	...
	function ProgressiveImage(singleImageData, index, pig) {
		...
		this.hash = singleImageData.hash;
	...
	ProgressiveImage.prototype.load = function() {
		...
		setTimeout(function() {
			...
			if (!this.thumbnail) {
				...
				this.thumbnail.src = this.pig.settings.urlForSize(this.filename, this.pig.settings.thumbnailSize, this.hash);
			...
			if (!this.fullImage) {
				...
				this.fullImage.src = this.pig.settings.urlForSize(this.filename, this.pig.settings.getImageSize(Number(this.getElement().style.width.replace("px", ""))), this.hash);
